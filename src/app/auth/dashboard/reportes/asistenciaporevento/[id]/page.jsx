      ...
      try {
        ...
        if (!codigoEvento) {
          Swal.fire({
            ...
            text: 'El codigo del Evento es necesario para generar el reporte.',
        ...
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_NEXTAUTH_URL}/api/reportes/asistenciaporevento/evento?codigoEvento=${codigoEvento}`
        );
        console.log("Datos de la API:", response.data.conteoNinosEnAsistencia);
        console.log("Datos de la API (asistencias):", response.data.asistencias);
        console.log("Total Asistentes (desde API):", response.data.totalAsistentes);
        ...
        if (Array.isArray(response.data.asistencias)) {
          ...
          setConteoNinosEnAsistencia(response.data.conteoNinosEnAsistencia || 0); // Asegurarse de que sea un número
        } else {
          console.warn("response.data.asistencias no es un array o está vacío.");
          setAsistencias([]);
          setTotalAsistentes(0);
          setConteoNinosEnAsistencia(0);
        }
        ...
        <PDFDownloadLink
          document={
            <ReportePDF 
              ...
              conteoNinosEnAsistencia={conteoNinosEnAsistencia}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {asistencias.length > 0 ? (
                asistencias.map((registroAsistencia) => (
                  <tr key={registroAsistencia.id_asi}> 
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {registroAsistencia.codigo_eve}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {registroAsistencia.nombre_eve}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {formatDate(registroAsistencia.fecha_eve)}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
